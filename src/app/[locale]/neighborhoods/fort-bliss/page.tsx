...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: valid, namespace: 'neighborhoods.guides.fortBliss' });
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  return renderPage(params, 'fortBliss', (locale) => (
    <NeighborhoodGuidePage locale={locale} neighborhoodKey="fortBliss" />
  ));
}
